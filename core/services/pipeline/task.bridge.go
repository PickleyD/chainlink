...
func (t *BridgeTask) Run(ctx context.Context, lggr logger.Logger, vars Vars, inputs []Result) (result Result, runInfo RunInfo) {
	...
	err = multierr.Combine(
		...
		// This param falls back to node config so explicitly passed in var is not essential
	...
	if t.Async == "true" {
		...
		// For LINKIT app
		requestData["responseURL"] = "BRIDGE_RESPONSE_URL_FROM_NODE_CONFIG"
	}
	...
	// TODO: Conditionally re-enable this when the user is able to pass in a URL through the UI
	...
	result = Result{Value: string(requestDataJSON)}
...
func (t BridgeTask) getBridgeURLFromName(name StringParam) (URLParam, error) {
	...
	// }
	parsed, err := url.Parse("http://bridge.url")

	if err != nil {
		return URLParam{}, err
	}

	return URLParam(*parsed), nil
}
