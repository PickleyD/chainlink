...
func (ks *eth) Enable(key ethkey.KeyV2, chainID *big.Int, qopts ...pg.QOpt) error {
	ks.lock.Lock()
	defer ks.lock.Unlock()
	return ks.enable(key, chainID, qopts...)
}
...
RETURNING *;`
	...
	if err := q.Get(state, sql, key.Address, chainID.String()); err != nil {
...
func (ks *eth) Disable(key ethkey.KeyV2, chainID *big.Int) error {
	...
	ks.lock.Lock()
	defer ks.lock.Unlock()
	ks.keyStates.disable(key.Address.Address(), chainID)
	return errors.Wrap(err, "failed to delete evm_key_state")
}

// Reset the key/chain nonce to the given one
func (ks *eth) Reset(key ethkey.KeyV2, chainID *big.Int, nonce int64) error {
	_, err := ks.orm.q.Exec(`UPDATE evm_key_states SET next_nonce = $1 WHERE address = $2 AND evm_chain_id = $3`, nonce, key.Address, chainID.String())
	if err != nil {
...
func (ks *eth) getV1KeysAsV2() (keys []ethkey.KeyV2, nonces []int64, _ error) {
	...
	for _, keyV1 := range v1Keys {
		...
		keyV2.IsFunding = keyV1.IsFunding
...
func (ks *eth) XXXTestingOnlySetState(state ethkey.State) {
	...
	*existingState = state
	sql := `UPDATE evm_key_states SET address = :address, next_nonce = :next_nonce, is_funding = :is_funding, evm_chain_id = :evm_chain_id, updated_at = NOW()
...
func (ks *eth) add(key ethkey.KeyV2, chainIDs ...*big.Int) (err error) {
	err = ks.safeAddKey(key, func(tx pg.Queryer) (serr error) {
		for _, chainID := range chainIDs {
			if serr = ks.enable(key, chainID, pg.WithQueryer(tx)); serr != nil {
				return serr
			}
		}
		return nil
...
func (ks *eth) addWithNonce(key ethkey.KeyV2, chainID *big.Int, nonce int64) (err error) {
	ks.lock.Lock()
	defer ks.lock.Unlock()
...
VALUES ($1, $2, false, $3, NOW(), NOW()) RETURNING *;`
		if err := ks.orm.q.Get(state, sql, key.Address, nonce, chainID); err != nil {
			return errors.Wrap(err, "failed to insert evm_key_state") // TODO: Rename all cases of evm_key_state
		}
